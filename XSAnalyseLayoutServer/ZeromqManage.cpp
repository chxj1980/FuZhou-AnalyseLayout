                            ...
                            LPSubMessageCallback pSubMessageCallback, void * pUser, int nThreadNum)
{
    ...
    {
        m_ThreadSubMessageID = CreateThread(NULL, 0, SubMessageThread, this, NULL, 0);   //订阅消息线程
    }
    for (int i = 0; i < m_nThreadNum; i++)
    {
        if (INVALID_HANDLE_VALUE == m_ThreadCallbackID[i])
        {
            m_ThreadCallbackID[i] = CreateThread(NULL, 0, MessageCallbackThread, this, NULL, 0);   //消息回调线程
        }
    }
#else
